...
```
1. u (own properties) ❌ not found
   ↓
2. u.__proto__ (User.prototype) ✅ found sayHi
```
...
1. u (own properties) ❌ not found
   ↓
2. u.__proto__ (User.prototype) ❌ not found
   ↓
3. User.prototype.__proto__ (Object.prototype) ✅ found toString
```
...
// Chain:
u → u.__proto__ → Object.prototype → null
...
```javascript
// ❌ WRONG - Calls parent constructor
...
console.log(u1.greet === u2.greet); // true ✅ Same function!
```

### Memory Comparison:

```javascript
// ❌ BAD - 1000 function copies in memory
...
console.log(user.hasOwnProperty('greet')); // false ❌ (in prototype)
...
console.log('greet' in user);    // true ✅ (in prototype)
console.log('toString' in user); // true ✅ (in Object.prototype)
console.log('fake' in user);     // false ❌
```
...
console.log(u1.age); // 30 ⚠️ Both affected!
console.log(u2.age); // 30 ⚠️
```
...
```javascript
// ❌ BAD - Changing prototype after creation is SLOW
const obj = {};
...
| `Object.getPrototypeOf()` | ✅ Recommended | Reading prototype |
| `Object.setPrototypeOf()` | ⚠️ Use carefully | Changing prototype (slow) |
| `Object.create()` | ✅ Recommended | Creating with prototype |
...
str (primitive)
 |
 └── Wrapped temporarily as String object
      |
...
```
Any Object → SpecificType.prototype → Object.prototype → null
```
...
console.log(u1.newMethod); // undefined ❌ (points to old prototype)
console.log(u2.newMethod); // function ✅ (points to new prototype)
```
...
```javascript
// ❌ BAD - Creates function for each instance
function User(name) {
  this.greet = function() {
    console.log(name);
  };
...
// 1000 instances = 1000 function copies ❌
...
const c = new Child();
console.log(c.constructor); // Parent ❌ Wrong!
...
- `x` in `inner()` → **scope chain** lookup → finds 30 in `outer()`
- `child.getX()` → **prototype chain** lookup → finds method in `parent`
- `this.x` inside `getX()` → `this` refers to `child`, so returns 20
...
|---------|-------------------|----------------------|
| Runs constructor | ✅ Yes | ❌ No |
| Sets prototype | ✅ Yes (`Constructor.prototype`) | ✅ Yes (specified proto) |
...
```
Own property:     FAST ⚡ (1 lookup)
Prototype:        MEDIUM 🐢 (2 lookups)
Deep prototype:   SLOW 🐌 (3+ lookups)
```
...
```javascript
// ❌ SLOW - Repeated prototype lookups
...
```javascript
// ❌ SLOW - Method accessed millions of times
...
console.log(user instanceof User); // false ❌ (different prototype now!)
```
...
console.log(Array.isArray(arr));   // true ✅ (reliable check)
```
...
console.log(u2.friends); // ['Alice'] ⚠️ Affected!
```
...
function User(name) {
  ...
  this.getName = function() {
    return name; // Closure keeps entire 'data' in memory! ❌
  };
}
...
```javascript
// ❌ WRONG - Calls parent constructor during setup
function Child() {}
...
greet(); // undefined ❌ (lost 'this' context)
...
2. `u2.name` still reads from prototype → `'Alice'`
3. `User.prototype.name` unchanged → `'Alice'`
4. After `delete u1.name`, own property removed, falls back to prototype → `'Alice'`
...
```javascript
// User case:
// return { custom: 'object' }; → returns this object
// u1 = { custom: 'object' }
...
1. ✅ Every object has `__proto__` (link), only constructor functions have `prototype` (blueprint)
2. ✅ `object.__proto__ === Constructor.prototype` is the fundamental relationship
3. ✅ `new` creates objects, links prototypes, runs constructors, and returns the result
4. ✅ Prototype chain enables inheritance through `__proto__` links until `null`
5. ✅ Methods on prototype are **shared** (memory efficient), properties on `this` are **unique**
6. ✅ `Object.create()` creates pure prototypal inheritance without constructors
7. ✅ ES6 classes are syntactic sugar over the prototype system
8. ✅ Never modify built-in prototypes in production code
9. ✅ Use `Object.getPrototypeOf()` instead of `__proto__` in modern code
10. ✅ Deep prototype chains have performance costs
...
Instance (created with 'new')
    ...
    |
    └→ lookup: own props → prototype → Object.prototype → null
```
